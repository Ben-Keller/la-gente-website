...
const placement = `photograph->{_id,title,alt,image{asset,crop,hotspot}},alt,localFraming{asset,crop,hotspot}`;
const video = `_id,title,url,webmUrl,mimeType,poster{${placement}}`;
export const CONTENT_QUERY = defineQuery(`{
  ...
  "pages": *[_type in ["homePage","aboutPage","chaptersPage","involvementPage","mediaPage","contactPage","pressPage","privacyPage","notFoundPage"]]{_id,route,title,seo{title,description,image{${placement}}},openingImages[]{${placement}},backgroundVideo->{${video}},trailer->{${video}},sections[]{_key,kind,label,heading,eyebrow,body,links,sourceSelector,images[]{${placement}}}},
  ...
  "videos": *[_type == "video" && !defined(supersededBy)]{${video}}
}`);
...
export function imageUrl(placement, width = 1600) {
  const shared = placement?.photograph?.image || placement?.image;
  // A changed photograph must never keep displaying an old locally-cropped asset.
  const image = placement?.localFraming?.asset?._ref === shared?.asset?._ref ? placement.localFraming : shared;
